...
exports.handler = async function (event, context) {
  ...
  // Fallback 数据
  const getFallbackWeeklyReport = (selectedCycle) => {
    ...
    return {
      ...
      advice:
        "1. 每次在进入高频博弈或工作宣讲会前，先默读一遍原则，并用舌头顶住上颚，用这一物理微动作提前切断防御应激。\n2. 在物理视角最显眼的地方贴上黄色小圆点，作为先记3个要点的强提醒，构建条件反射。",
    };
  };
  ...
  if (!apiKey || apiKey === "MY_GEMINI_API_KEY") {
    ...
    return {
      ...
      body: JSON.stringify({ ...getFallbackWeeklyReport(body.cycle), isSimulated: true }),
    };
  }
...
请严格遵循以下 JSON 架构进行返回，不要带有 \`\`\`json 标记，只要纯净的 valid JSON String：
{
  ...
  "advice": "行动建议的多行拼接文本..."
    ...
    return {
      statusCode: 200,
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ...parsed, isSimulated: false }),
    };
  } catch (err) {
